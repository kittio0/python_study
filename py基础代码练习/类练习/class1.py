...
class jiaowu:
    ...
    # 读取一个 0-100 的整数成绩:输入字母、空内容等不会让程序崩溃,而是提示后重新输入
    def input_score(self, tip):
        while True:
            try:
                score = int(input(tip))
            except ValueError:
    ...
    def add_student(self):
        ...
        for s in self.students:
            if s.name == name:
                print('学生已存在')
                return
        chinese = self.input_score('请输入语文成绩:')
        math = self.input_score('请输入数学成绩:')
        english = self.input_score('请输入英语成绩:')
        ...
        while True:
            ...
            for s in self.students:
                if s.name == name:
                    ...
                    chinese = self.input_score('请输入新的语文成绩:')
                    math = self.input_score('请输入新的数学成绩:')
                    english = self.input_score('请输入新的英语成绩:')
                    s.chinese = chinese
                    s.math = math
                    s.english = english
                    print('修改成功')
